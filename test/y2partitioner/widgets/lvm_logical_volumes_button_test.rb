...
describe Y2Partitioner::Widgets::LvmLogicalVolumesButton do
  ...
  describe "#handle" do
    ...
    context "when the option for editing the LVs is selected" do
      ...
      it "creates an action to jump to the 'Partitions' tab of the device" do
        expect(Y2Partitioner::Actions::GoToDeviceTab)
          .to receive(:new).with(device, pager, "Log&ical Volumes")
        button.handle(event)
      end
    ...
    context "when the option for adding an LV is selected" do
      let(:selected_option) { :"#{widget_id}_add" }

      before do
        allow(Y2Partitioner::Actions::AddLvmLv).to receive(:new).and_return action
      end

      it "opens the workflow for adding a logical volume" do
        expect(Y2Partitioner::Actions::AddLvmLv).to receive(:new)
        button.handle(event)
      end
    ...
    context "when the option for deleting all LVs is selected" do
      let(:selected_option) { :"#{widget_id}_delete" }

      before do
        allow(Y2Partitioner::Actions::DeleteLvmLvs).to receive(:new).and_return action
      end

      it "calls the action for removing all logical volumes" do
        expect(Y2Partitioner::Actions::DeleteLvmLvs).to receive(:new)
        button.handle(event)
      end
