...
describe Y2Partitioner::Actions::DeleteNfs do
  before do
  ...
  let(:filesystem) { device_graph.nfs_mounts.find { |f| f.mount_path == mount_path } }
  ...
  describe "#run" do
    ...
    shared_examples "do not delete" do
      it "does not delete the NFS" do
        subject.run

        fs = device_graph.nfs_mounts.find { |f| f.mount_path == mount_path }
    ...
    context "when the confirm message is accepted" do
      ...
      context "when the NFS is not mounted in the system" do
        let(:mount_path) { "/test1" }

        it "does not ask for unmounting the NFS" do
      ...
      context "when the NFS is mounted in the system" do
        let(:mount_path) { "/test2" }
        ...
        it "asks for unmounting the NFS" do
